...
public sealed class AxionApp : IDisposable
{
    ...
    /// <summary>
    /// Optional project / asset root used to resolve relative script paths and other
    /// content references.  Hosts (e.g. the editor) should set this once at startup.
    /// </summary>
    ...
    {
        // Resolve any ScriptComponents that couldn't load earlier (scene was
        // deserialized before AxionApp.Instance existed, or before the per-script
        // ScriptPath was filled in).  This MUST happen before the Behavior pass
        // below — Attach() adds the freshly-compiled Behavior to the GameObject,
        // and we want OnAttach to run for it in this same awake cycle.
        foreach (var sc in Scene.AllOfType<ScriptComponent>())
            sc.EnsureLoaded();
