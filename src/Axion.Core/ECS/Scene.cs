...
public sealed class Scene
{
    ...
    {
        // Snapshot to a list so callers may add or remove components during
        // iteration (e.g. ScriptComponent.EnsureLoaded adding a GelBehavior
        // while AwakeAllBehaviorsInScene is enumerating).
        var result = new List<T>();
        foreach (var (t, list) in _byType)
            if (typeof(T).IsAssignableFrom(t))
                foreach (var c in list) if (c is T ct) result.Add(ct);
        return result;
    }
